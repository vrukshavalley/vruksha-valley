    ...
    <div>
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          ...
          <p className="text-gray-400 text-sm font-mono">{form.slug}</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSave} className="space-y-6">
        ...
        <div className="bg-white rounded-lg shadow-sm p-6 space-y-5">
          ...
          <Field label="Keywords (comma separated)"><input value={form.seo_keywords} onChange={e => set('seo_keywords', e.target.value)} className={inp} /></Field>
        </div>
        ...
        <div className="flex gap-4">
          ...
          <Link href="/admin/blog" className="px-8 py-3 bg-gray-100 text-gray-700 text-sm rounded hover:bg-gray-200 transition">Back</Link>
        </div>
      </form>
    </div>
  )
...
function Field({ label, children, cls = '' }: any) {
  return (
    <div className={cls}>
